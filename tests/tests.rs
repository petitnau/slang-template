...
fn test_verifier(_path: &Utf8Path, content: String) -> datatest_stable::Result<()> {
    ...
    if !extra_errors.is_empty() && !missing_errors.is_empty() {
        Err(format!("Extra errors in lines: {}. Missing errors in lines: {}.", extra_errors.iter().join(", "), missing_errors.iter().join(", ")).into())
    } 
    else if extra_errors.is_empty() && !missing_errors.is_empty() {
        Err(format!("Missing errors in lines: {}.", missing_errors.iter().join(", ")).into())
    } 
    else if !extra_errors.is_empty() && missing_errors.is_empty() {
